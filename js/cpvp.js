document.addEventListener('DOMContentLoaded', function () {
	let players = [];
	const _loadEl = document.getElementById('cpvp-tabulka');
	...
	fetch('https://docs.google.com/spreadsheets/d/e/2PACX-1vTsYd1Hv8XjsdskgT2O-_Otwe3DKxXTXECPE0s4JcPwPPnLMMpknU_-y8EHNBZTtVEQgzicFKcgluSU/pub?output=xlsx')
		...
		.then(data => {
			...
			players = rows.filter(row => row.UUID && row.Nick).map(row => ({
				...
				cpvp: normalizeTier(row.Crystal)
			}));
			renderCpvpTable(players);
		});
	...
	function renderCpvpTable(players) {
		const tabulka = document.getElementById('cpvp-tabulka');
		if (!tabulka) return;
		tabulka.innerHTML = '';
		...
		for (const tierObj of tiers) {
			...
			// Nejprve HT hráči
			players.filter(p => p.cpvp === tierObj.ht).forEach(player => {
			...
			// Poté LT hráči
			players.filter(p => p.cpvp === tierObj.lt).forEach(player => {
				const div = document.createElement('div');
		...
		var tc = players.filter(function(p){ return p.cpvp !== ''; }).length;
