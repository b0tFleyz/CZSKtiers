...
function renderKitPage(slug, columnKey) {
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderKitTable(slug, players) {
      ...
      const tiers = [
        { name: 'Tier 1', color: 'var(--accent, #eecd14)', icon: '🥇', ht: 'HT1', lt: 'LT1' },
      ...
      for (const tierObj of tiers) {
        ...
        players.filter(p => p.tier === tierObj.ht).forEach(player => {
          list.appendChild(createPlayerDiv(player, false));
        });
        // LT players
        players.filter(p => p.tier === tierObj.lt).forEach(player => {
          list.appendChild(createPlayerDiv(player, true));
        });
        col.appendChild(list);
        columns.appendChild(col);
      }
      tabulka.appendChild(columns);
      // Update player count badge with kit-specific count
      var testedCount = players.filter(function(p) { return p.tier !== ''; }).length;
    ...
    function createPlayerDiv(player, isLt) {
      const div = document.createElement('div');
      div.className = 'kit-player' + (isLt ? ' kit-player-lt' : '');
