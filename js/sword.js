document.addEventListener('DOMContentLoaded', function () {
	let players = [];
	const _loadEl = document.getElementById('sword-tabulka');
	...
	fetch('https://docs.google.com/spreadsheets/d/e/2PACX-1vTsYd1Hv8XjsdskgT2O-_Otwe3DKxXTXECPE0s4JcPwPPnLMMpknU_-y8EHNBZTtVEQgzicFKcgluSU/pub?output=xlsx')
		...
		.then(data => {
			...
			players = rows.filter(row => row.UUID && row.Nick).map(row => ({
				...
				sword: normalizeTier(row.Sword)
			}));
			renderSwordTable(players);
		});
	...
	function renderSwordTable(players) {
		const tabulka = document.getElementById('sword-tabulka');
		if (!tabulka) return;
		tabulka.innerHTML = '';
		...
		for (const tierObj of tiers) {
			...
			// Nejprve HT hráči
			players.filter(p => p.sword === tierObj.ht).forEach(player => {
			...
			// Poté LT hráči
			players.filter(p => p.sword === tierObj.lt).forEach(player => {
				const div = document.createElement('div');
		...
		var tc = players.filter(function(p){ return p.sword !== ''; }).length;
