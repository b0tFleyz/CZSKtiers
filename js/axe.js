document.addEventListener('DOMContentLoaded', function () {
	let players = [];
	const _loadEl = document.getElementById('axe-tabulka');
	...
	function renderAxeTable(players) {
		const tabulka = document.getElementById('axe-tabulka');
		if (!tabulka) return;
		tabulka.innerHTML = '';
		...
		for (const tierObj of tiers) {
			...
			// Nejprve HT hráči
			players.filter(p => p.axe === tierObj.ht).forEach(player => {
			...
			// Poté LT hráči
			players.filter(p => p.axe === tierObj.lt).forEach(player => {
				const div = document.createElement('div');
		...
		var tc = players.filter(function(p){ return p.axe !== ''; }).length;
