document.addEventListener('DOMContentLoaded', function () {
	let players = [];
	const _loadEl = document.getElementById('uhc-tabulka');
	...
	function renderUhcTable(players) {
		const tabulka = document.getElementById('uhc-tabulka');
		if (!tabulka) return;
		tabulka.innerHTML = '';
		...
		for (const tierObj of tiers) {
			...
			// Nejprve HT hráči
			players.filter(p => p.uhc === tierObj.ht).forEach(player => {
			...
			// Poté LT hráči
			players.filter(p => p.uhc === tierObj.lt).forEach(player => {
				const div = document.createElement('div');
		...
		var tc = players.filter(function(p){ return p.uhc !== ''; }).length;
