document.addEventListener('DOMContentLoaded', function () {
	let players = [];
	const _loadEl = document.getElementById('diasmp-tabulka');
	...
	fetch('https://docs.google.com/spreadsheets/d/e/2PACX-1vTsYd1Hv8XjsdskgT2O-_Otwe3DKxXTXECPE0s4JcPwPPnLMMpknU_-y8EHNBZTtVEQgzicFKcgluSU/pub?output=xlsx')
		...
		.then(data => {
			...
			players = rows.filter(row => row.UUID && row.Nick).map(row => ({
				...
				diasmp: normalizeTier(row.DiaSMP)
			}));
			renderDiasmpTable(players);
		});
	...
	function renderDiasmpTable(players) {
		const tabulka = document.getElementById('diasmp-tabulka');
		if (!tabulka) return;
		tabulka.innerHTML = '';
		...
		for (const tierObj of tiers) {
			...
			// Nejprve HT hráči
			players.filter(p => p.diasmp === tierObj.ht).forEach(player => {
			...
			// Poté LT hráči
			players.filter(p => p.diasmp === tierObj.lt).forEach(player => {
				const div = document.createElement('div');
		...
		var tc = players.filter(function(p){ return p.diasmp !== ''; }).length;
